...
export default {
    ...
    reporters: [
        ...
        // Reference: https://github.com/jemishgopani/wdio-json-reporter
        [
            'json',
            {
                outputDir: './results',
    ...
    before: async function (capabilities: any, specs: any) {
        ...
        try {
            ...
            // Log the assert object to verify it's properly initialized
            logger.info('Chai assertion library initialized successfully')
        } catch (error) {
            logger.error('Failed to initialize Chai:', error)
        }

        // Set global API log level
        globalThis.apiLogLevel = apiLogLvl
        logger.info(`wdio.conf.ts :: API_LOG_LEVEL :: ${globalThis.apiLogLevel}`)
    },

    beforeTest: function (test: { title: any }, _context: any) {
        console.log(`\x1b[36mRunning:\x1b[0m \x1b[90m${test.title}\x1b[0m`)
    },
    /**
